...
import commons.api.CoreServiceM;
...
extends AbstractSample {
	...
	protected CoreServiceM coreServiceM;
	...
	public AbstractCoreServiceSample() {
		...
		coreServiceM = new CoreServiceM(host, instance, tenant, user, password);
	...
	throws IOException {
		Device device;
		try {
			device = coreServiceM.getDevice(deviceId, gateway);
			...
			device = coreServiceM.addDevice(deviceTemplate);
	...
	throws IOException {
		...
		sensor = coreServiceM.addSensor(sensorTemplate);
	...
	throws IOException {
		...
		SensorType[] existingSensorTypes = coreServiceM.getSensorTypes();
		...
		SensorType sensorType = coreServiceM.addSensorType(sensorTypeTemplate);
	...
	throws IOException {
		Capability[] existingCapabilities = coreServiceM.getCapabilities();
		...
		Capability capability = coreServiceM.addCapability(capabilityTemplate);
